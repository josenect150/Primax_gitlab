...
class XlsxWriter {
  ...
  protected function writeStylesXml() {
    ...
    // $file->write(        '<numFmt formatCode="[$$-1009]#,##0.00;[RED]\-
    // [$$-1009]#,##0.00" numFmtId="165"/>');
    // $file->write(        '<numFmt formatCode="YYYY-MM-DD\ HH:MM:SS"
    // numFmtId="166"/>');
    ...
    foreach ($fonts as $font) {
      // Fonts have 4 empty placeholders in array to offset the 4 static
      // xml entries above.
      if (!empty($font)) {
    ...
    foreach ($fills as $fill) {
      // Fills have 2 empty placeholders in array to offset the 2 static xml
      // entries above.
      if (!empty($fill)) {
    ...
    foreach ($borders as $border) {
      // Fonts have an empty placeholder in the array to offset the static
      // xml entry above.
      if (!empty($border)) {
    ...
    // $file->write(        '<xf applyAlignment="false" applyBorder="false"
    // applyFont="false" applyProtection="false" borderId="0" fillId="0"
    ...
    foreach ($style_indexes as $v) {
      ...
      // $file->write('<xf applyAlignment="'.$applyAlignment.'" applyBorder="'
      // .$applyBorder.'" applyFont="'.$applyFont.'" applyProtection="false"
      // borderId="'.($borderIdx).'" fillId="'.($fillIdx).'" fontId="
      // '.($fontIdx).'" numFmtId="'.(164+$v['num_fmt_idx']).'" xfId="0"/>');
...
/**
 * XlsxWriterBuffererWriter class manager.
 */
